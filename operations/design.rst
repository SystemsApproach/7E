...
as opposed to any single device, works as expected.
...
routes updates they had processed, and so on—back to the mangement
system, with with the goal of detching problematic behavior and
...
statistical anomoly is detected. And in the limit, we could build a
*closed control loop*, where an analyics alert triggers a new intent,
...
.. figure:: operations/figures/mgmt-system.png
   ...
   Idealized management system, prividing an intent-based interface
...
focus of this chapter. And as our thought experiment suggests, we can
think of the mangement system as having two parts: one responsible for
configuring network devices and the other responsible for monitoring
network devices.  Consider each, in turn.
...
programmatic interface that supports ``GET`` and ``PUT`` operations,
...
the devices (that program responds to ``GET`` and ``PUT`` requests),
...
.. [#] This discussion of provisioning implicitly assumes that
       ...
       on a broadband provider. For this broader perspective, see
       the discussion in Section |Ops|.4.

Another aspect of the problem is *inventory management*, that is,
planning and tracking both the physical (racks, servers, switches,
cabling) and virtual (IP ranges and addresses, VLANs) resources in a
...
*Lifecycle Management*, that is, the process of bringing up resources
on Day 1, maintaining those resources on Days 2 through N (potentially
spanning multiple upgrades), and decommisioning (or releasing) those
resources on Day N+1.
...
record their activity (such as number of packets sent or rececived) in
local counters.  Layered on top those raw meters/counters is a data
collection mechanism that periodically reads the meters and records
their values in a timeseries database. We also need a query mechanism
that allows operators to look at the data, as well as to feed
dashboards that display the data graphically. Finally, we need
analysis tools that sift through the data, helping operators diagnose
...
decisions. As a general rule, the answer is that there are three kinds
of telemetry data: metrics, logs, and traces. 
...
understood in a single program, but in a cloud setting, a trace is
...
management, except for observed behavior as opposed to operational
parameters. Note that we can treat some of the variables read by the
configuration system as telemetry data—that is, there are some
variables that serve both parts—but in general, it's good practice to
keep the two schemas distinct.
