...
.. [#] Believe it or not, there was also a paper book (like a phone
       ...
       connected to the Internet and all the people who had an
...
top of the hierarchy. Without going into that discussion in any
detail, the hierarchy in the early days was not very wide at the first
level. There are domains for each country, plus the “big six” domains:
``.edu``, ``.com``, ``.gov``, ``.mil``, ``.org``, and ``.net``. These
six domains were all originally based in the United States (where the
Internet and DNS were invented); for example, only U.S.-accredited
educational institutions can register an ``.edu`` domain name. Over
the last several years, the number of top-level domains (TLDs) has
been greatly expanded, partly to deal with the demand for ``.com``
domain names. The newer top-level domains include ``.biz``, ``.coop``,
and ``.systems``. There are around 1500 TSDs at the time of writing.
...
Numbers (ICANN). Below the root is a zone corresponding to each of the
TLDs, and below them are zones for companies and organizations, such
as Princeton University. Within the Princeton zone, some departments
do not want the responsibility of managing the hierarchy (and so they
remain in the university-level zone), while others, like the
Department of Computer Science, manage their own department-level
zone.
...
this example). It also has ``A`` records that translate these names
...
::
   ...
   (edu, a.edu-servers.net, NS, IN)
   (a.edu-servers.net, 192.5.6.32, A, IN)
...
::
   ...
   (princeton.edu, dns1.princeton.edu, NS, IN)
   (dns1.princeton.edu, 128.112.0.10, A, IN)
   ...
...
::
   ...
   (dns1.cs.princeton.edu, 128.112.155.10, A, IN)
   ...
...
::
   ...
   (cicada.cs.princeton.edu, 128.112.155.166, A, IN)
   (www.cs.princeton.edu, coreweb.cs.princeton.edu, CNAME, IN)
   (coreweb.cs.princeton.edu, 128.112.155.35, A, IN)
   (cs.princeton.edu, mail.cs.princeton.edu, MX, IN)
   (mail.cs.princeton.edu, 128.112.155.72, A, IN)
   ...
...
``cicada.cs.princeton.edu`` relative to the set of servers given in
...
server ``a.edu-servers.net``. The server also returns all records that are
related to this record, in this case, the ``A`` record for
``a.edu-servers.net``. The client, having not received the answer it was
...
before to the server at IP host ``128.112.0.10``, and this time gets
...
``128.112.155.10`` yields the ``A`` record for
``cicada.cs.princeton.edu``, and the client learns that the
corresponding IP address is ``128.112.155.166``.

This example still leaves a couple of questions about the resolution
process unanswered. The first question is how the client locates
...
long each record can be safely cached. In :numref:`Figure %s
<fig-resolution>`, for example, we show the local server's first
request going to the TLD server for ``.edu``. This would likely
happen, in practice, because it was able to match ``.edu`` based on
information cached from an earlier lookup.

The second question is how the system works when a user submits a
partial name (e.g., ``cicada``) rather than a complete domain name
(e.g., ``cicada.cs.princeton.edu``). The answer is that the client
...
.. _fig-resolution:
.. figure:: naming/figures/resolution.png
   :width: 600px
   :align: center

   Name resolution in practice, where the numbers 1 to 8 show the sequence
